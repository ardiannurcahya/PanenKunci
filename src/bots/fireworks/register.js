...
const CONFIG = {
  ...
  outputFile: process.env.WORKER_OUTPUT_FILE || path.join(__dirname, '../../../output/fireworks.csv'),
...
function loadEmails() {
  if (process.env.WORKER_EMAILS) {
    return process.env.WORKER_EMAILS.split(',').map(e => e.trim()).filter(Boolean);
  }
...
async function registerOne(page, email, index, total) {
  ...
  const tag = WID ? `[W${WID} ${index + 1}/${total}]` : `[${index + 1}/${total}]`;
...
// ─── MAIN ────────────────────────────────────────────────
async function main() {
  const wLabel = WID ? `[Worker #${WID}] ` : '';
  console.log(`=== ${wLabel}Fireworks AI Auto-Registration Bot ===\n`);
