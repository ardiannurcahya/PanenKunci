...
const CONFIG = {
  ...
  outputFile: path.join(__dirname, '../../../output/fireworks3.csv'),
  passwordFile: path.join(__dirname, '../../../data/password.txt'),
...
function loadAccounts() {
  if (!fs.existsSync(CONFIG.passwordFile)) {
    console.error(`Password file not found: ${CONFIG.passwordFile}`);
    process.exit(1);
  }
  const lines = fs.readFileSync(CONFIG.passwordFile, 'utf8')
    .split('\n')
    .map(l => l.trim())
    .filter(l => l.length > 0);
  ...
  if (accounts.length === 0) {
    console.error('No accounts found in password.txt');
    process.exit(1);
  }
  return accounts;
}
...
async function registerOne(page, email, password, index, total) {
  ...
  saveToCsv(CONFIG.outputFile, email, password, '');
  ...
  try {
    ...
    updateCsvApiKey(CONFIG.outputFile, email, 'ERROR');
    return false;
  }
}
...
async function main() {
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    const success = await registerOne(page, accounts[i].email, accounts[i].password, i, accounts.length);
    if (success) {
      successCount++;
    } else {
      failCount++;
